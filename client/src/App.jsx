...
export default function App() {
  ...
  const [modelState,  setModelState]  = useState('stopped'); // stopped | starting | running
  const [controlling, setControlling] = useState(false);

  // Poll model state every 5s so UI stays in sync with idle shutdowns
  useEffect(() => {
    const poll = () =>
      fetch('/api/health')
        .then(r => r.json())
  ...
  const analyze = async () => {
    ...
    // Model may be stopped — server will auto-start it; reflect that in UI
    ...
    try {
      ...
      if (image.type === 'file') {
        ...
        body.append('image_url', image.url);
      }
